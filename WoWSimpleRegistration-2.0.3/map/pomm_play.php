...
// Ensure the character set is set correctly
$characters_db->query("SET NAMES 'utf8'");
$query = $characters_db->query("SELECT `memberGuid` FROM `group_member` WHERE `memberGuid` IN(SELECT `guid` FROM `characters` WHERE `online`='1')");
if ($query) {
    while ($result = $characters_db->fetch_assoc($query)) {
        $groups[$result['memberGuid']] = $result['memberGuid'];
    }
}
...
while ($result = $characters_db->fetch_assoc($query)) {
    ...
    $arr[$i]['memberGuid'] = isset($groups[$char_data]) ? $groups[$char_data] : 0;
    $i++;
}
$characters_db->close();
unset($characters_db);
